		...
		if (currentResults.poseLandmarks && currentResults.poseLandmarks.length > 0) {
			...
			try {
				...
				POSE_CONNECTIONS.forEach(([start, end]) => {
					if (
						start < currentResults.poseLandmarks.length &&
						end < currentResults.poseLandmarks.length
					) {
	...
	<div class="video-container" bind:this={videoContainer}>
		{#if active && !error}
			<div class="display-wrapper">
				...
				<video bind:this={videoElement} autoplay muted playsinline class="hidden-video"></video>

				<!-- Main display canvas -->
				<canvas bind:this={displayCanvas} class="display-canvas"></canvas>
	...
	<div class="panel-info">
		...
		{#if currentResults}
			...
			<div class="info-row">
				<span>Hands:</span>
				<span
					class:detected={currentResults.leftHandLandmarks || currentResults.rightHandLandmarks}
				>
					{currentResults.leftHandLandmarks || currentResults.rightHandLandmarks
						? 'Detected'
						: 'None'}
				</span>
			</div>
