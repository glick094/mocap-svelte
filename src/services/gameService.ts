...
export class GameService {
	...
	// Collision detection
	public checkCollisions(data: PoseData): {
		hit: boolean;
		hitType?: string;
		modeProgress?: any;
		playSound?: boolean;
	...
	private checkHipSwayCollisions(data: PoseData): {
		hit: boolean;
		hitType?: string;
		modeProgress?: any;
		playSound?: boolean;
		...
		switch (this.state.hipSwayState.phase) {
			case 'centering':
				if (isCentered) {
					if (!this.state.hipSwayState.isCentered) {
						...
						const centeringDuration =
							Date.now() - (this.state.hipSwayState.centeringStartTime || 0);
			...
			case 'targeting':
				...
				if (
					inTargetRegion &&
					!this.state.hipSwayState.inTargetRegion &&
					!this.state.hipSwayState.trialCompleted
				) {
					...
					const isComplete =
						(this.state.hipSwayState.leftSideHits >= targetsPerSide &&
					...
					if (isComplete) {
						...
						setTimeout(() => {
							...
							const currentSideHits =
								this.state.hipSwayState.targetSide === 'left'
									? this.state.hipSwayState.leftSideHits
									: this.state.hipSwayState.rightSideHits;
							...
							if (currentSideHits >= targetsPerSide) {
								...
								this.state.hipSwayState.targetSide =
									this.state.hipSwayState.leftSideHits < this.state.hipSwayState.rightSideHits
										? 'left'
										: 'right';
							} else {
								...
								this.state.hipSwayState.targetSide =
									this.state.hipSwayState.targetSide === 'left' ? 'right' : 'left';
							}
	...
	private checkHandsCenteringCollisions(data: PoseData): {
		hit: boolean;
		hitType?: string;
		modeProgress?: any;
		playSound?: boolean;
		...
		switch (handsState.phase) {
			case 'centering':
				if (handsState.currentTrial === 1) {
					...
					const leftHandCentered = this.checkHandCollisionAtPosition(
						data,
						handsState.leftCenterX,
						handsState.leftCenterY,
						handsState.centeringTolerance
					);
					const rightHandCentered = this.checkHandCollisionAtPosition(
						data,
						handsState.rightCenterX,
						handsState.rightCenterY,
						handsState.centeringTolerance
					);
					...
					const activeHandCentered = this.checkSpecificHandCollisionAtPosition(
						data,
						activeHand,
						centerX,
						centerY,
	...
	private checkHeadCenteringCollisions(data: PoseData): {
		hit: boolean;
		hitType?: string;
		modeProgress?: any;
		playSound?: boolean;
			...
			case 'centering':
				...
				if (data.faceLandmarks && data.faceLandmarks[1]) {
					...
					const distance = Math.sqrt(
						Math.pow(headX - headState.centerX, 2) + Math.pow(headY - headState.centerY, 2)
					);
	...
	private checkFixedTargetCollisions(data: PoseData): {
		hit: boolean;
		hitType?: string;
		modeProgress?: any;
		playSound?: boolean;
		...
		if (
			this.state.fixedTargets.length === 0 ||
	...
	private checkRandomTargetCollisions(data: PoseData): {
		hit: boolean;
		hitType?: string;
		hitKeypoint?: string;
		playSound?: boolean;
		...
		switch (this.state.currentTarget.type) {
			...
			case TARGET_TYPES.HEAD:
				if (data.faceLandmarks && data.faceLandmarks.length > 0) {
					...
					if (
						nose &&
						this.checkDistance(nose.x * this.width, nose.y * this.height, targetX, targetY)
					) {
						targetHit = true;
						hitKeypoint = 'face_1';
					}
				...
				if (!targetHit && data.poseLandmarks && data.poseLandmarks[0]) {
					...
					if (
						poseNose &&
						this.checkDistance(poseNose.x * this.width, poseNose.y * this.height, targetX, targetY)
					) {
						targetHit = true;
						hitKeypoint = 'pose_0';
					}
			...
			case TARGET_TYPES.KNEE:
				if (data.poseLandmarks) {
					...
					if (
						leftKnee &&
						this.checkDistance(leftKnee.x * this.width, leftKnee.y * this.height, targetX, targetY)
					) {
						targetHit = true;
						hitKeypoint = 'pose_25';
					}
					if (
						...
						rightKnee &&
						this.checkDistance(
							rightKnee.x * this.width,
							rightKnee.y * this.height,
							targetX,
							targetY
						)
					) {
		...
		if (targetHit && !this.state.hitTargetIds.has(this.state.currentTarget.id)) {
			...
			if (this.state.currentTarget.type in this.state.scoreBreakdown) {
				this.state.scoreBreakdown[
					this.state.currentTarget.type as keyof typeof this.state.scoreBreakdown
				]++;
			}
			...
			return {
				...
				hitKeypoint: hitKeypoint || undefined,
				playSound: true
			};
		}
	...
	private checkHandCollision(data: PoseData, targetX: number, targetY: number): boolean {
		if (data.leftHandLandmarks) {
			for (let landmark of data.leftHandLandmarks) {
				if (
					landmark &&
					this.checkDistance(landmark.x * this.width, landmark.y * this.height, targetX, targetY)
				) {
					return true;
				}
			}
	...
	private checkSpecificHandCollisionAtPosition(
		data: PoseData,
		hand: 'left' | 'right',
		targetX: number,
		targetY: number,
	...
	private checkHandSpecificTargetCollisions(data: PoseData): {
		hit: boolean;
		hitType?: string;
		modeProgress?: any;
		playSound?: boolean;
		...
		if (handsState.primaryHand === null && this.state.currentFixedTargetIndex === 1) {
			...
			for (let i = 0; i < landmarks.length; i++) {
				...
				if (
					landmark &&
					this.checkDistance(landmark.x * this.width, landmark.y * this.height, target.x, target.y)
				) {
	...
	private checkHandCollisionAtPosition(
		data: PoseData,
		targetX: number,
		targetY: number,
		tolerance: number
		...
		if (data.leftHandLandmarks) {
			for (let landmark of data.leftHandLandmarks) {
				if (landmark) {
					const distance = Math.sqrt(
						(landmark.x * this.width - targetX) ** 2 + (landmark.y * this.height - targetY) ** 2
					);
					if (distance <= tolerance) {
						return true;
					}
	...
	private createHipSwayTargetData(targetSide: 'left' | 'right'): void {
		...
		const virtualTarget = {
			...
			color:
				targetSide === 'left'
					? TARGET_COLORS[TARGET_TYPES.HIP_LEFT]
					: TARGET_COLORS[TARGET_TYPES.HIP_RIGHT]
		};
	...
	private createHandsCenteringData(): void {
		...
		const primaryText =
			handsState.primaryHand === null
				? 'detection'
				: activeHand === handsState.primaryHand
					? 'primary'
					: 'secondary';
