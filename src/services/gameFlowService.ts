...
export class GameFlowService {
  ...
  public stopFlow(): void {
    ...
    if (this.callbacks.onRecordingStop && this.state.currentGame) {
      this.callbacks.onRecordingStop(this.state.currentGame, this.getGameDataFilename(this.state.currentGame));
    }
  }
  ...
  private startNextGame(): void {
    ...
    console.log(`Starting game ${this.state.currentGameIndex + 1}/${this.config.games.length}: ${this.state.currentGame}`);
    
    // Start recording for new game
    if (this.config.autoStartRecording && this.callbacks.onRecordingStart && this.state.currentGame) {
      this.callbacks.onRecordingStart(this.state.currentGame, this.getGameDataFilename(this.state.currentGame));
    }
  ...
  private startDelay(): void {
    ...
    const nextGame = nextGameIndex < this.config.games.length ? this.config.games[nextGameIndex] : null;
    
    if (nextGame) {
      console.log(`Starting delay: ${this.config.delayBetweenGames / 1000}s until next game (${nextGame})`);
