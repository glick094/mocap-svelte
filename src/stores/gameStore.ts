...
export const gameSettings = writable<GameSettings>({
	...
	hipSwayTextPrompts: {
		...
		targeting: {
			...
			instruction: (side: string) => `Move your hips to the ${side.toUpperCase()} rectangle`
		},
		completed: {
			main: 'Hip Sway Game Complete!',
			score: (score: number, total: number) => `Final Score: ${score}/${total}`
		}
	},
	gameModeTexts: {
		displayNames: {
...
export const gameProgress = derived([gameScore, totalTargets], ([$score, $total]) =>
